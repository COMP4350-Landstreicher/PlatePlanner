...
const getByName = asyncHandler(async (recipeName, userID, Recipe) => {
  ...
  const recipe = await Recipe.findOne({
    where: {recipeName: recipeName, userID: userID},
  });
...
// Get the shopping list for all recipes with
// portion size >0 for the current user
...
const getShoppingListRecipes = asyncHandler(async (userID, Recipe) => {
  ...
  const recipes = await Recipe.findAll({
    ...
    where: {userID: userID, portion: {[Op.gt]: '0'}},
  });
