...
export function genMailList(ingredients) {
  let stringBuilder = 'mailto:?subject=Shopping%20List&body=' + encodeURIComponent('Shopping List: \r\n');

  for (const i in ingredients) { // Check that the items are valid
    if (!Array.isArray(ingredients) ||
      !('ingredientName' in ingredients[i] &&
        'totalAmount' in ingredients[i] &&
        'ingredientUnit' in ingredients[i])
    ) {
      throw new Error('ingredients malformed!');
    } else {
      stringBuilder += encodeURIComponent(ingredients[i]['ingredientName'] + ' - ' +
        ingredients[i]['totalAmount'] + ' ' + ingredients[i]['ingredientUnit'] + '\r\n');
    }
  }
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <Grid
        container
        spacing={2}
        sx={{maxWidth: `calc(100%- 250px)`, ml: '260px'}}
      >
        <Grid item xs={8}>
